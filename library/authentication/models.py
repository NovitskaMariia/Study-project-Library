...
class CustomUser(AbstractBaseUser):
    ...
    secret_key = models.CharField(max_length=100, default=None, null=True)
    ...
    @staticmethod
    def create(email, password, first_name=None, middle_name=None, last_name=None, role=0, secret_key = None):
        """
        ...
        if len(first_name) <= 20 and len(middle_name) <= 20 and len(last_name) <= 20 and len(email) <= 100 and len(
                email.split('@')) == 2 and len(CustomUser.objects.filter(email=email)) == 0 and not secret_key:
            ...
            custom_user.save()
            return custom_user
        elif len(first_name) <= 20 and len(middle_name) <= 20 and len(last_name) <= 20 and len(email) <= 100 and len(
                email.split('@')) == 2 and len(CustomUser.objects.filter(email=email)) == 0 and secret_key == KEY_FOR_LIBRARIAN:
            custom_user = CustomUser(email=email, password=password, first_name=first_name, middle_name=middle_name,
